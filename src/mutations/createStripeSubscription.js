...
export default async function createStripeSubscription(context, input) {
    ...
    let customers = await stripe.customers.list({ email: customerEmail.toLowerCase() });
    ...
    if (customer) {
        ...
        const paymentAttachResponse = await stripe.paymentMethods.attach(paymentMethodId, { customer: customer.id });
        ...
        const update = { $set: { subscriptionStatus: "active", updatedAt: new Date() } };
        ...
        const subscriptionTableResp = await StripeSubscription.findOne({ customerId: userID });
        if (subscriptionTableResp) {
            ...
            const options = { new: true };
            const StripeSubscriptionResponse = await StripeSubscription.findOneAndUpdate(
                filter,
                update,
                options
            );
            ...
            const SubscriptionResponse = await StripeSubscription.insertOne(newSubscription);
