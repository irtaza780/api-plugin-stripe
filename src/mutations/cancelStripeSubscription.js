...
export default async function cancelStripeSubscription(context, input) {
    ...
    if (userInfo?.subscriptionStatus) {
        const SubscriptionResponse = await StripeSubscription.findOne({ "customerId": input });
        // console.log("SubscriptionResponse ", SubscriptionResponse);
        const subscription = await stripe.subscriptions.retrieve(SubscriptionResponse.subscriptionId);
        ...
        if (subscription.status == 'past_due' || subscription.status == 'unpaid') {
            ...
            const cancelSubscription = await stripe.subscriptions.update(SubscriptionResponse.subscriptionId, {
